...
inline float SinF(float ang) {
    return SinFIdx(NW4R_MATH_IDX_TO_FIDX(ang));
}
inline f32 SinIdx(short idx) {
    return SinF(U16ToF32(idx));
}
...
inline float CosF(float ang) {
    return CosFIdx(NW4R_MATH_IDX_TO_FIDX(ang));
}
inline f32 CosIdx(short idx) {
    return CosF(U16ToF32(idx));
}
