...
namespace nw4r { namespace snd { namespace detail
{
	...
	public:
		...
		// vtable MmlParser
		virtual void CommandProc(MmlSeqTrack *track, ulong command,
		...
		static ulong ParseAllocTrack(void const *baseAddress, ulong seqOffset,
		                           ulong *allocTrack);

	private:
		u8 ReadByte(byte_t const **ptr) const { return *(*ptr)++; }
		u16 Read16(byte_t const **ptr) const;
		ulong Read24(byte_t const **ptr) const;
