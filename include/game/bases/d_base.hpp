 ...
 * @details
 * ## Overview
 * From a practical standpoint, dBase_c does not add any particularly relevant components. A few overloaded
 * functions from the fManager_c search API (::searchBaseByProfName) and the fBase_c base creation API
 * (::createBase, ::createRoot) are provided for convenience.
 *
 * ## Base Linking
 ...
 * the linked bases. When the owner base is deleted, all the linked bases are unlinked.
 *
 * Several bases use this method to link themselves to dBgGm_c, which however does not use the set at all.
 *
 * ## Unused Content
 * ### Debug Strings
 * Bases store a pointer to a "kind string" (which roughly equals to the @ref fBase_c::GROUP_TYPE_e
 * "group type") and to a @ref fProfile::PROFILE_NAME_e "profile name" string.
 * The former can be obtained by calling ::getKindString, while the latter can be obtained by calling
 * ::dProf_getName with the profile name.
 *
