...
class dBoyon_c {
public:
    dBoyon_c(dActor_c *owner) {
        mScale = mVec3_c(1.0f, 1.0f, 1.0f);
        mFactor = 1.0f;
        mCounter = 0;
        mpOwner = owner;
    }
    void init(int i, float v);

    static float smc_INITIAL_FACTOR;
    static int smc_INITIAL_COUNTER;
...
public:
    ...
    bool isState(sStateIDIf_c &other) {
