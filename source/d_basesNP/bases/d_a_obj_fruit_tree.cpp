...
int daObjFruitTree_c::create() {
    ...
    if (style == STYLE_REGULAR) {
        const char *fileName = "g3d/fruits_kusa.brres";
        res = dResMng_c::m_instance->mRes.getRes("fruits_kusa", fileName);
    } else {
        const char *fileName = "g3d/fruits_kusa_gake.brres";
        res = dResMng_c::m_instance->mRes.getRes("fruits_kusa_gake", fileName);
    }
