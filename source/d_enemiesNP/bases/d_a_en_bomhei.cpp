...
const sBcSensorPoint l_bomhei_head = { SENSOR_IS_POINT, 0x0, 0x10000 };
const sBcSensorLine l_bomhei_foot = { SENSOR_IS_LINE, -0x3000, 0x3000, 0 };
const sBcSensorPoint l_bomhei_wall = { SENSOR_IS_POINT, 0x6000, 0x6000 };
...
const sCcDatNewF l_bomhei_cc = {
    ...
    BIT_FLAG(CC_KIND_PLAYER) | BIT_FLAG(CC_KIND_PLAYER_ATTACK) | BIT_FLAG(CC_KIND_YOSHI) |
        BIT_FLAG(CC_KIND_ITEM) | BIT_FLAG(CC_KIND_ENEMY) | BIT_FLAG(CC_KIND_TAMA),
...
void explosionCallback(dCc_c *cc1, dCc_c *cc2) {
    ...
    if (other->mKind == dActor_c::STAGE_ACTOR_PLAYER || other->mKind == dActor_c::STAGE_ACTOR_YOSHI) {
        ...
        player->setDamage(self, daPlBase_c::DAMAGE_DEFAULT);
    }
}
...
void daEnBomhei_c::initializeState_Carry() {
    ...
    x = mCc.mCcData.mBase.mOffset.x;
    y = mCc.mCcData.mBase.mOffset.y;
    width = mCc.mCcData.mBase.mSize.x;
    height = mCc.mCcData.mBase.mSize.y;
    mCcOffsetX = x;
    mCcOffsetY = y;
    mCcWidth = width;
    mCcHeight = height;
...
void daEnBomhei_c::executeState_AfterIce() {
    ...
    switch (m_23b) {
        ...
        case 2:
            ...
            if (mAngle.y.chaseAngle(l_base_angleY[mDirection], 0x400)) {
                changeState(StateID_Walk);
            }
            break;
    }
...
void daEnBomhei_c::initializeState_Explode() {
    ...
    mCc.mCcData.mBase.mSize.x = 18.0f;
    mCc.mCcData.mBase.mSize.y = 18.0f;
    mCc.mCcData.mAttack = CC_ATTACK_SHELL;
