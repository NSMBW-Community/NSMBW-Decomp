...
dEn_c::dEn_c() : m24(0), mFootRelated(mVec3_c(0.0f, 0.0f, 0.0f)), m46(false), mFootAttr3(false), mFootAttr1(false),
mBlockHit(), mBoyon(this), mIceMng(this), mCounterCont(1), mFumiProc(this) {
...
int dEn_c::preExecute() {
    ...
    if (!mWasSquished && !isState(StateID_Ice) && isQuakeDamage()) {
...
void dEn_c::fumidamageSE(const mVec3_c &, int) {
    dAudio::playEmySound(SE_EMY_DOWN, mPos, 0);
}
...
void dEn_c::posMove() {
    mAng ang(mBc.getSakaMoveAngle(mSpeed.x < 0.0f));
    mVec3_c move;
    if (mFootAttr3) {
        move.set(0.0f, 0.0f, 0.0f);
    } else {
        float speed = fabs(mSpeed.x);
        float cos = ang.cos();
        float sin = ang.sin();
        move.x = speed * cos;
        ...
        if (m49) {
            ...
            bool ok = mBlockHit.mData.mIsInit || mWasSquished;
            if (!ok && mSpeed.y <= 0.0f) {
...
void dEn_c::checkWallAndBg() {
    ...
    truePos += mCenterOffs;
    mBc.checkBg(truePos.x, truePos.y, mLayer, l_Ami_Line[mUnkBool], 0x819);
}
    ...
    if (treadCount >= 0) {
        ...
        switch (mCounterCont.mClapMode) {
            case 1: {
                ...
                instance->FUN_800e2070(pos, treadCount, *actor->getPlrNo(), 1);
                break;
            }
            case 2: {
                dScoreMng_c *instance = dScoreMng_c::getInstance();
                instance->FUN_800e2310(pos, treadCount, *actor->getPlrNo());
                break;
            }
        }
    }
...
void dEn_c::fumiSE(dActor_c *actor) {
    ...
    cs_combo_se.playEmySound(count, centerPos, 0);
}
...
void dEn_c::mamefumiSE(dActor_c *actor) {
    mVec3_c centerPos = getCenterPos();
    dAudio::playEmySound(SE_EMY_MAME_STEP, centerPos, 0);
}

void dEn_c::fumistepSE(dActor_c *actor) {
    mVec3_c centerPos = getCenterPos();
    dAudio::playEmySound(SE_EMY_CMN_STEP, centerPos, 0);
}

void dEn_c::yoshifumistepSE(dActor_c *actor) {
    mVec3_c centerPos = getCenterPos();
    dAudio::playEmySound(SE_EMY_YOSHI_STEP, centerPos, 0);
}
...
u32 dEn_c::EnBgCheck() {
    ...
    if (mBc.mFlags & 0x8000 && mBc.mFlags & 3 && mBc.rcRef != nullptr) {
        int v = 0;
        if (mSpeed.y > 0.0f) {
            v |= 1;
        }
        if (!mBc.rcRef->check2(actArg & 0x1f6000, v, 0)) {
...
void dEn_c::WaterCheck(mVec3_c &pos, float h) {
    ...
    if (waterLineRes == 3) {
        ...
        m46 = true;
    } else if (waterLineRes != 0 && mWasSquished) {
        m46 = true;
    }
}
...
bool dEn_c::LineBoundaryCheck(dActor_c *actor) {
    daPlBase_c *pl = (daPlBase_c *) actor;
    if ((pl->mPos.z > 0.0f && mUnkBool == 1) || (pl->mPos.z < 0.0f && mUnkBool == 0)) {
...
bool dEn_c::Area_XY_check(float x, float y) {
    for (int i = 0; i < 4; i++) {
        ...
        if (player != nullptr && daPyMng_c::checkPlayer(i)) {
            mVec3_c tmp = player->mPos;
            if (fabsf(tmp.x - mPos.x) < x && fabsf(tmp.y - mPos.y) < y) {
                return true;
            }
        }
    }
...
void dEn_c::setNicePoint_Death() {
    int v = (s8) mBlockHit.mData.m1d;
    if (mBlockHit.mData.m1d > 3) {
        return;
    }
...
void dEn_c::boyonInit() {
    mBoyon.mScale = mBoyon.mpOwner->mScale;
}

void dEn_c::boyonBegin() {
    mBoyon.init(dBoyon_c::smc_INITIAL_COUNTER, dBoyon_c::smc_INITIAL_FACTOR);
}
