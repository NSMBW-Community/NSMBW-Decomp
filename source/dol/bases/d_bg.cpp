...
void dBg_c::CoinGetBitSet(u16 p1, u16 p2, int p3) {
    ...
    a |= b;
    dBgParameter_c::ms_Instance_p->setOneGetBuff(dScStage_c::m_instance->mCurrCourse, c, p3, a);
}
...
bool dBg_c::CheckExistLayer(u8 layer) {
    ...
    switch (layer) {
        case 0:
            return true;
        case 1:
            if (g->mData[stage->mCurrCourse].mLayer1 != 0) {
                return true;
            }
            break;
        case 2:
            if (g->mData[stage->mCurrCourse].mLayer2 != 0) {
                return true;
            }
            break;
    }
...
bgTex_c *dBg_c::__createBgTex(int layer, u16 tw, u16 th, u16 w, u16 h, int i1, int i2) {
    ...
    dBgUnit_c *bgUnit = bgGlobal->GetBgUnitP(stage->mCurrCourse, layer);
    if (bgUnit != nullptr) {
        tex = new(m_FrmHeap_p, 4) bgTex_c();
        tex->create(tw, th, w, h, i1, i2, mHeap::g_gameHeaps[0], layer);
        bool check = false;

        char *env = bgGlobal->getEnvironment(stage->mCurrCourse, 1);
        if (env[0] != '\0' &&
...
void dBg_c::initAutoScroll() {
    ...
    if (stage->mCurrCourse != STAGE_1) return;
    ...
    if (m_bg_p->m_9095a == 0) {
        ...
        if (dScStage_c::m_loopType != 2) {
            ...
            if (dScStage_c::m_instance->m_120c == WORLD_2 &&
                ...
                dScStage_c::m_instance->mCurrCourse == 0 &&
                m_90958 == 0
            ) {
                if (x < leftLimit) {
                    x = leftLimit;
...
void dBg_c::fn_8007ac40(dBgSomeInfo_c *info, int arg1) {
    ...
    if (mLimitRelated == 1 || mLimitRelated == 4) {
        ...
        if (dScStage_c::m_instance->mCurrWorld == 11 && dScStage_c::m_instance->mCurrCourse == 65 && dScStage_c::m_instance->mCurrArea == 1) {
            w -= 48.0f;
        }
    }
    bool check = false;
    ...
    if (m_9095c == 0) {
        if ((stage->mCurrCourse != STAGE_DOOMSHIP || stage->m_120d != 2) && stage->mCurrCourse != STAGE_CASTLE) {
...
void dBg_c::initScroll() {
    ...
    for (int i = 0; i < 4; i++) {
        mVec3_c pos = daPyMng_c::getPlayerSetPos(stage->mCurrCourse, stage->mCurrWorld);
        float x = pos.x;
        float y = pos.y;
        if (x < playerBoundL) {
            playerBoundL = x;
