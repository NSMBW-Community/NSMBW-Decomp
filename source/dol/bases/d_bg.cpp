...
void dBg_c::bg_createHeap() {
    if (m_FrmHeap_p == nullptr) {
        m_FrmHeap_p = mHeap::createFrmHeap(m_HeapSize.mSize, EGG::Heap::sCurrentHeap, nullptr, 0x20, 0);
    }
}
...
dBg_c::dBg_c() {
    ...
    for (int i = 0; i < ARRAY_SIZE(mWaveRelated); i++) {
...
dBgUnit_c *dBg_c::fn_80077520(u16 x, u16 y, u8 layer, int *param_5, bool b) {
    int currCourse = dScStage_c::m_instance->currCourse;
    dBgUnit_c *bgUnit = dBgGlobal_c::ms_pInstance->GetBgUnitP(currCourse, layer);
...
void dBg_c::CoinGetCommon(u16 tileIndex, u16 cellIndex, int someCounter, u16* out1, u16* out2, u16* out3) {
    int course = dScStage_c::m_instance->currCourse;
...
void dBg_c::CoinGetBitSet(u16 p1, u16 p2, int p3) {
    ...
    a |= b;
    dBgParameter_c::ms_Instance_p->setOneGetBuff(dScStage_c::m_instance->currCourse, c, p3, a);
}
...
bool dBg_c::CheckExistLayer(u8 layer) {
    ...
    switch (layer) {
        case 0:
            return true;
        case 1:
            if (g->mData[stage->currCourse].mLayer1 != 0) {
                return true;
            }
            break;
        case 2:
            if (g->mData[stage->currCourse].mLayer2 != 0) {
                return true;
            }
            break;
    }
...
void dBg_c::CreateBgCheckBuffer() {
    ...
    for (int i = 0; i < 4; i++) {
        char *env = dBgGlobal_c::ms_pInstance->getEnvironment(currCourse, i);

        if (env[0] != '\0') {
            sprintf(path, "BG_chk/d_bgchk_%s.bin", env);
            sprintf(resName, "%s", env);

            unsigned long long *data = (unsigned long long *) dResMng_c::m_instance->mRes.getRes(resName, path);
            for (int j = 0; j < 0x100; j++) {
                unsigned long long *unit = getpBgCheckBuff(i * 0x100 + j);
                *unit = data[j];
            }
        }
    }
...
bgTex_c *dBg_c::__createBgTex(int layer, u16 tw, u16 th, u16 w, u16 h, int i1, int i2) {
    ...
    dBgUnit_c *bgUnit = bgGlobal->GetBgUnitP(stage->currCourse, layer);
    if (bgUnit != nullptr) {
        tex = new(m_FrmHeap_p, 4) bgTex_c();
        tex->create(tw, th, w, h, i1, i2, mHeap::g_gameHeaps[0], layer);
        bool check = false;

        char *env = bgGlobal->getEnvironment(stage->currCourse, 1);
        if (env[0] != '\0' &&
...
u16 dBg_c::dBg_getUpLimitScroll(u8 layer) {
    dCdFile_c *file = dCd_c::m_instance->getFileP(dScStage_c::m_instance->currCourse);
    u8 id = file->getAreaID(layer);
    u8 scrollID = file->getAreaScroll(id);
    return file->getScrollDataP(scrollID)->mUpLimit;
}

int dBg_c::dBg_getScrlAreaDataSize(u8 layer) {
    dCdFile_c *file = dCd_c::m_instance->getFileP(dScStage_c::m_instance->currCourse);
    return file->mScrlAreaDataSize;
}

dScrollData_c *dBg_c::dBg_getScrlAreaDataP(u8 layer, u8 idx) {
    dCdFile_c *file = dCd_c::m_instance->getFileP(dScStage_c::m_instance->currCourse);
...
void dBg_c::calcLookatOffsLimit() {
    ...
    if (dBg_isCloudFlyPlayer() == 1 && (!mU8s[4] || mAutoscrolls[0].mActive)) {
        smth1 = 88.0f;
        smth2 = 88.0f;
        smth1 += file->getScrollDataP(mScrollID)->mXCloudMaybe;
        smth2 -= file->getScrollDataP(mScrollID)->mYCloudMaybe;
    } else {
        smth1 += file->getScrollDataP(mScrollID)->mXMaybe;
        smth2 -= file->getScrollDataP(mScrollID)->mYMaybe;
    }
...
void dBg_c::initAutoScroll() {
    ...
    m_9008c = 0;
    m_9008d = 1;
    if (stage->m_120c != 1) return;
    if (stage->m_120d != 23) return;
    if (stage->currCourse != STAGE_1) return;
...
void dBg_c::calcAutoScroll() {
    ...
    dRailInfo_c *ri = dRail_c::getRailInfoP(mAutoscrolls[0].m_14);
    dScStage_c *stage = dScStage_c::m_instance;
    dCdFile_c *file = dCd_c::m_instance->getFileP(stage->currCourse);
    ...
    bool cond1 = true;
    bool cond2 = true;
    if (stage->m_120c == WORLD_2 && stage->m_120d == STAGE_CASTLE && stage->currCourse == 0) {
        calcLoopAutoScroll();
    ...
    if (m_bg_p->m_9095a == 0) {
        ...
        if (dScStage_c::m_loopType != 2) {
            ...
            if (dScStage_c::m_instance->m_120c == WORLD_2 &&
                dScStage_c::m_instance->m_120d == STAGE_CASTLE &&
...
void dBg_c::fn_8007ac40(dBgSomeInfo_c *info, int arg1) {
    ...
    if (mLimitRelated == 1 || mLimitRelated == 4) {
        ...
        if (dScStage_c::m_instance->currWorld == 11 && dScStage_c::m_instance->currCourse == 65 && dScStage_c::m_instance->mCurrArea == 1) {
            w -= 48.0f;
        }
    }
    bool check = false;
    ...
    if (m_9095c == 0) {
        if ((stage->currCourse != STAGE_DOOMSHIP || stage->m_120d != 2) && stage->currCourse != STAGE_CASTLE) {
...
void dBg_c::fn_8007cd70(dBgSomeInfo_c *info1, dBgSomeInfo_c *info2, int i1) {
    ...
    if (dScStage_c::m_loopType == 2 || i1 == 1) {
        ...
        if (stage->currWorld == WORLD_8 && stage->currCourse == STAGE_7 || cond) {
            fVar6 = 6.0f;
            fVar7 = 0.5f;
            fVar5 *= 6.0f;
        } else {
