...
void dBg_c::bg_createHeap() {
    ...
    m_FrmHeap_p = mHeap::createFrmHeap(
        m_HeapSize.mSize, EGG::Heap::getCurrentHeap(),
...
void dBg_c::CreateBgCheckBuffer() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (env[0] != '\0') {
            ...
            nw4r::g3d::ResFile data = dResMng_c::m_instance->getRes(resName, path);
...
int dBg_c::dBg_isCloudFlyPlayerMulti() {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (pl->mScrollMode == 6) {
            ...
            if (pl->mPc.m_30.y > param->mPos.y - 160.0f && mDispScale < 0.7f
                || mDispScale >= 0.7f && pl->mPc.m_30.y > param->mPos.y - 48.0f) {
                res = true;
            }
        }
    }
...
void dBg_c::calcLoopAutoScroll() {
    ...
    float actualY = bgParam->ySize() + getDownLimit();
...
void dBg_c::calcAutoScroll() {
    ...
    float downLim = getDownLimit() + bgH;
    float upLim = getUpLimit();
...
void dBg_c::calcScroll(const mVec3_c &pos, int param_2) {
    ...
    if (m_bg_p->m_9095a == 0) {
        ...
        float downLimit = getDownLimit() + height;
...
void dBg_c::fn_8007ac40(const dBgSomeInfo_c *info, int arg1) {
    ...
    if ((daPyDemoMng_c::mspInstance->mFlags & 1) == 0) {
        ...
        if (mMoreFloats5[0] == tgMax) {
            if (mMoreFloats5[2] != tgMid && m_90018 >= mMoreFloats5[1]) {
                mMoreFloats5[1] = m_90018;
            }
    ...
    if (fnres2 > mPrevSomePos.y) {
        if (m_90009 == 1 || m_9000a == 1) {
            iVar9 = 1;
            boundScale = fnres2;
    ...
    if (iVar9 != 0) {
        ...
        } else if (m_90009 == 1 || m_9000a == 1) {
            boundScale = fnres2;
        }
    }
    if (iVar9 != 0) {
        if ((int) boundScale < (int) getPrevSomePosY()) {
            float diff = getPrevSomePosY() - boundScale;
            if (diff < 16.0f) {
                mMoreFloats5[3] -= diff;
            } else {
                mMoreFloats5[3] -= 16.0f;
            }
        } else if ((int) boundScale > (int) getPrevSomePosY()) {
            float diff = boundScale - getPrevSomePosY();
            if (diff < 16.0f) {
                mMoreFloats5[3] += diff;
            } else {
                mMoreFloats5[3] += 16.0f;
    ...
    float up = info->mBounds.getU();
    float down = info->mBounds.getD();
    if (up > getPrevSomePosY() + 32.0f) {
        up = getPrevSomePosY() + 32.0f;
    }
    ...
    float fVar3 = getPrevSomePosY() - getPrevSomeSizeY() * 0.5f;
    ...
    if (!mAutoscrolls[0].mActive) {
        mAutoscrolls[0].mPos.x = getPrevSomePosX();
        mAutoscrolls[0].mPos.y = getPrevSomePosY();
    }
}
...
bool dBg_c::fn_8007bc40(const dBgSomeInfo_c *info, float f) {
    ...
    float fVar3 = 112.0f - bgParam->getM78();
    float fVar4 = fVar3;
    if (fVar1 < ll + fVar4) {
        fVar3 = fVar1 - ll;
    }
    float fVar5 = 112.0f - bgParam->getM78();
    if (fVar2 > rl - fVar5) {
        fVar4 = rl - fVar2;
    }
    if (info->mBounds.getW() > f * mVideo::getLayoutWidth() - (fVar3 + fVar4)) {
        return true;
    } else {
        return false;
    }
...
mVec2_c dBg_c::fn_8007bd40(const dBgSomeInfo_c *info, float f1, float f2) {
    ...
    float ll = getLeftLimit();
    float rl = getRightLimit();
    float uLim = getUpLimit();
    float dLim = getDownLimit();
    ...
    float resX = (offsX1 + baseX + offsX2) / layoutWidth;
    if (resX < 1.0f) {
        resX = 1.0f;
    } else if (resX > f1) {
        resX = f1;
    ...
    float tmp = offsY1 + baseY + offsY2;
    float resY = tmp * f2 / layoutHeight;
    if (resY < 1.0f) {
        resY = 1.0f;
    } else if (resY > f1) {
        resY = f1;
...
bool isUpPlayer(dAcPy_c *pl1, dAcPy_c *pl2) {
    return pl1->mPc.m_30.y > pl2->mPc.m_30.y;
}
...
void dBg_c::fn_8007ca90(dBgSomeInfo_c *info, int i1, int i2) {
    ...
    switch (i1) {
        case 0:
            if (pl1 != nullptr) {
                info->m_14 = pl1;
                info->mBounds.mRight = fn_8007c7d0(pl1->mPc.m_30.x);
                info->m_24 = pl1->mPlayerType;
            }
            if (pls[i2 - 1] != nullptr) {
                info->m_10 = pls[i2 - 1];
                info->mBounds.mLeft = fn_8007c7d0(pl2->mPc.m_30.x);
        ...
        case 2:
            if (pl2 != nullptr) {
                info->m_14 = pl2;
                info->mBounds.mRight = fn_8007c7d0(pl2->mPc.m_30.x);
                info->m_24 = pl2->mPlayerType;
            }
            if (pls[0] != nullptr) {
                info->m_10 = pls[0];
                info->mBounds.mLeft = fn_8007c7d0(pl1->mPc.m_30.x);
        ...
        case 1:
            if (pl1 != nullptr) {
                info->m_18 = pl1;
                info->mBounds.mUp = pl1->mPc.m_30.y;
                info->m_28 = pl1->mPlayerType;
            }
            if (pls[i2 - 1] != nullptr) {
                info->m_1c = pls[i2 - 1];
                info->mBounds.mDown = pl2->mPc.m_30.y;
        ...
        case 3:
            if (pl2 != nullptr) {
                info->m_18 = pl2;
                info->mBounds.mUp = pl2->mPc.m_30.y;
                info->m_28 = pl2->mPlayerType;
            }
            if (pls[0] != nullptr) {
                info->m_1c = pls[0];
                info->mBounds.mDown = pl1->mPc.m_30.y;
...
void dBg_c::fn_8007cd70(dBgSomeInfo_c *info1, dBgSomeInfo_c *info2, int i1) {
    ...
    bool cond;
    bool cond2;
    dBgParameter_c *bgParam2 = dBgParameter_c::getInstance();
    px = bgParam2->mPos.x;
    py = bgParam2->mPos.y;
    sx = bgParam2->mSize.x;
    sy = bgParam2->mSize.y;
    ...
    info1->m_2c = info2->m_2c;
    info1->m_30 = info2->m_30;
    float bl = info1->mBounds.getL();
    float l = info2->mBounds.getL();
    float br = info1->mBounds.getR();
    float r = info2->mBounds.getR();
    float bu = info1->mBounds.getU();
    float u = info2->mBounds.getU();
    float bd = info1->mBounds.getD();
    float d = info2->mBounds.getD();
    float tmpx = (bl + br) * 0.5f;
    float tmpy = (bu + bd) * 0.5f;
    dBgParameter_c *bgParam = dBgParameter_c::getInstance();
    ...
    if (dScStage_c::m_loopType == 2 || i1 == 1) {
        ...
        if (u > bu && u > py - fVar4 || d < bd && d < fVar4 + (py - sy)) {
            cond = true;
        }
        w1 = (info1->mBounds.getR() + info1->mBounds.getL()) * 0.5f;
        h1 = (info1->mBounds.getU() + info1->mBounds.getD()) * 0.5f;
        w2 = (info2->mBounds.getR() + info2->mBounds.getL()) * 0.5f;
        h2 = (info2->mBounds.getU() + info2->mBounds.getD()) * 0.5f;
        len = sqrt((w2 - w1) * (w2 - w1) + (h2 - h1) * (h2 - h1));
        if (len != 0.0f) {
            dw = (w2 - w1) / len;
            dh = (h2 - h1) / len;
        } else {
        ...
        } else if (l > bgParam->xStart() + bgParam->xSize() * 0.5f) {
            sLib::addCalc(&mIdkF[0], 0.0f, calcArg1, calcArg2, calcArg3);
        } else {
            sLib::addCalc(&mIdkF[0], f17, calcArg1, calcArg2, calcArg3);
        }
        tmp1 = mIdkF[0] * (f23 - calcScale) + calcScale;
        tmp2 = fVar12 + mIdkF[0] * (fVar5 - fVar12);
        sLib::addCalc(&bl, l, tmp1, tmp2, 0.1f);

        if (r > br) {
            sLib::addCalc(&mIdkF[1], 0.0f, calcArg1, calcArg2, calcArg3);
        } else if (r < bgParam->xStart() + bgParam->xSize() * 0.5f) {
            sLib::addCalc(&mIdkF[1], 0.0f, calcArg1, calcArg2, calcArg3);
        } else {
            sLib::addCalc(&mIdkF[1], f17, calcArg1, calcArg2, calcArg3);
        }
        tmp1 = mIdkF[1] * (f23 - calcScale) + calcScale;
        tmp2 = fVar12 + mIdkF[1] * (fVar5 - fVar12);
        sLib::addCalc(&br, r, tmp1, tmp2, 0.1f);

        if (u > bu) {
            sLib::addCalc(&mIdkF[2], 0.0f, calcArg1, calcArg2, calcArg3);
        } else if (u < bgParam->yStart() - bgParam->ySize() * 0.5f) {
            sLib::addCalc(&mIdkF[2], 0.0f, calcArg1, calcArg2, calcArg3);
        } else {
            sLib::addCalc(&mIdkF[2], f17, calcArg1, calcArg2, calcArg3);
        }
        tmp1 = mIdkF[2] * (fVar7 - calcScale) + calcScale;
        tmp2 = fVar13 + mIdkF[2] * (f21 - fVar13);
        sLib::addCalc(&bu, u, tmp1, tmp2, 0.1f);

        if (d < bd) {
            sLib::addCalc(&mIdkF[3], 0.0f, calcArg1, calcArg2, calcArg3);
        } else if (d > bgParam->yStart() - bgParam->ySize() * 0.5f) {
            sLib::addCalc(&mIdkF[3], 0.0f, calcArg1, calcArg2, calcArg3);
        } else {
            sLib::addCalc(&mIdkF[3], f17, calcArg1, calcArg2, calcArg3);
        }
        tmp1 = mIdkF[3] * (fVar7 - calcScale) + calcScale;
        tmp2 = fVar13 + mIdkF[3] * (f21 - fVar13);
        sLib::addCalc(&bd, d, tmp1, tmp2, 0.1f);

        info1->mBounds.mLeft = bl;
        info1->mBounds.mRight = br;
        info1->mBounds.mUp = bu;
        info1->mBounds.mDown = bd;
    }
    ...
    if (px <= m_bg_p->getLeftLimit() || px + sx >= m_bg_p->getRightLimit()) {
        m_bg_p->m_900b8 = 0.0f;
    } else {
        m_bg_p->m_900b8 = h_lr - tmpx;
    }
    if (py >= m_bg_p->getUpLimit() || py - sy <= m_bg_p->getDownLimit()) {
        m_bg_p->m_900bc = 0.0f;
    } else {
        m_bg_p->m_900bc = h_ud - tmpy;
    }
}
