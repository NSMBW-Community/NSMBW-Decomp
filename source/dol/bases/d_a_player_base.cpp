...
int daPlBase_c::create() {
    mKey.init();
    if (dScStage_c::m_gameMode == dInfo_c::GAME_MODE_TITLE) {
...
int daPlBase_c::preExecute() {
    ...
    offStatus(STATUS_B7);
    offStatus(STATUS_40);
    offStatus(STATUS_JUMP_COUNT_INCREASED);
...
void daPlBase_c::postExecute(fBase_c::MAIN_STATE_e status) {
    if (status == SUCCESS) {
        ...
        offStatus(STATUS_SLOPE_HEAD_PUSH_L);
        offStatus(STATUS_SLOPE_HEAD_PUSH_R);
        offStatus(STATUS_BE);
    ...
    if (isNowBgCross(BGC_FOOT)) {
        if (mpMdlMng->isAnmStop()) {
            if (!mKey.buttonDown()) {
                ...
                setHipBlockBreak();
                if (!wasStaffCreditGroundHipAttack) {
                    if (isNowBgCross(BGC_COIN_BLOCK_HIT)) {
                        mSubstateTimer = 15;
                    }
                    if (dScStage_c::m_isStaffCredit && isNowBgCross(BGC_FOOT) && !isNowBgCross(BGC_LIFT)) {
                        wasStaffCreditGroundHipAttack = 1;
                    }
                }
                if (mSubstateTimer == 0) {
                    offStatus(STATUS_HIP_ATTACK);
...
void daPlBase_c::finalizeState_HipAttack() {
    ...
    offStatus(STATUS_7F);
    offStatus(STATUS_7A);
    wasStaffCreditGroundHipAttack = 0;
}
...
void daPlBase_c::grandPowerSet() {
    ...
    } else if (isStatus(STATUS_SLIP_ACTIVE)) {
        slipPowerSet(1);
    } else {
        slipPowerSet(0);
    }
...
void daPlBase_c::calcWindSpeed() {
    ...
    if (isStatus(STATUS_A0) && !isOnSinkSand()) {
        ...
        if (isNowBgCross(BGC_FOOT)) {
            if (isStatus(STATUS_WIND_AIRBORNE)) {
                offStatus(STATUS_WIND_AIRBORNE);
                mSpeedF += mWindSpeed;
                mWindSpeed = 0.0f;
            }
            float scale;
...
void daPlBase_c::setSoundPlyMode() {
    ...
    mSndObj.m_04 = scPlayerSound[mPowerup];
}

void daPlBase_c::setFootSound() {
    if (
        (isDemo() || isNowBgCross(BGC_FOOT)) &&
        dScStage_c::m_gameMode != dInfo_c::GAME_MODE_TITLE &&
...
bool daPlBase_c::setDemoOutDokanAction(int entranceNextGotoID, DokanDir_e dir) {
    ...
    if (nextGoto->mFlags & NEXT_GOTO_FLAG_RAIL) {
        mDokanMode = DEMO_DOKAN_RAIL;
    } else if (nextGoto->mFlags & NEXT_GOTO_FLAG_WATER_TANK) {
    ...
    switch (mDokanMode) {
        case DEMO_DOKAN_NORMAL:
            ...
            if (nextGoto->mType == NEXT_GOTO_TYPE_WATER_TANK) {
...
void daPlBase_c::initializeState_DemoRailDokan() {
    ...
    if (nextGoto->mFlags & NEXT_GOTO_FLAG_RAIL_REVERSE) {
...
void daPlBase_c::setExitRailDokan() {
    ...
    switch (nextGoto->mType) {
        case NEXT_GOTO_TYPE_PIPE_U:
            // @bug This should be StateID_DemoInDokanD.
            changeDemoState(StateID_DemoInDokanU, DOKAN_TYPE_CONNECTED);
            break;
        case NEXT_GOTO_TYPE_PIPE_D:
            changeDemoState(StateID_DemoInDokanU, DOKAN_TYPE_CONNECTED);
            break;
...
void daPlBase_c::executeDemoGoal_Pole() {
    switch ((DemoGoalState_Pole_e) mDemoState) {
        ...
        case GOAL_DEMO_WAIT_BELOW_PLAYER:
            if (isStatus(STATUS_GOAL_POLE_CAN_SLIDE)) {
...
bool daPlBase_c::isActionRevisionY() {
    if (isStatus(STATUS_POLE) || isStatus(STATUS_TARZAN_ROPE)) {
        return true;
    }
    return false;
}
...
void daPlBase_c::checkBgCross() {
    ...
    if (bgFlags & 0x1fe000) {
        ...
        if (bgFlags & 0x800000) {
            onNowBgCross(BGC_COIN_BLOCK_HIT);
        }
    ...
    if ((bgFlags & 8) || isStatus(STATUS_SLOPE_HEAD_PUSH_L)) {
        onNowBgCross(BGC_WALL_TOUCH_L_2);
    }
    if ((bgFlags & 4) || isStatus(STATUS_SLOPE_HEAD_PUSH_R)) {
...
void daPlBase_c::postBgCross() {
    ...
    if (isNowBgCross(BGC_FOOT)) {
        ...
        } else if (isSaka() || isStatus(STATUS_SLIP_ACTIVE)) {
            mSpeed.y = 0.0f;
        } else {
            mSpeed.y = -2.0f;
        }
        ...
        if (!isNowBgCross(BGC_LIFT) && isNowBgCross(BGC_HEAD)) {
            if (mBc.getSakaType() != 0 && mBc.getSakaUpDown(mDirection) == 1) {
                if (mDirection == DIR_LR_R) {
                    onStatus(STATUS_SLOPE_HEAD_PUSH_R);
                } else {
                    onStatus(STATUS_SLOPE_HEAD_PUSH_L);
                }
                mSpeedF = 0.0f;
                mPos.x = mLastPos.x;
            }
