...
void dGameDisplay_c::RestDispSetup() {
    ...
    static const PLAYER_TYPE_e lbl_802F5C28[PLAYER_COUNT] = {
        PLAYER_MARIO,
        PLAYER_LUIGI,
        PLAYER_BLUE_TOAD,
        PLAYER_YELLOW_TOAD,
...
void dGameDisplay_c::Effect1UP(int a) {
    if (
        ...
        dInfo_c::m_startGameInfo.mGameMode == dInfo_c::GAME_MODE_TITLE_REPLAY
    ) {
        return;
    }
    if (m_424[a] == 0) {
...
void dGameDisplay_c::setCoinNum(int num_coins) {
    if (!m_453 && (mCoins != num_coins)) {
        ...
        if (
            dInfo_c::m_startGameInfo.mGameMode == dInfo_c::GAME_MODE_TITLE ||
            dInfo_c::m_startGameInfo.mGameMode == dInfo_c::GAME_MODE_TITLE_REPLAY
        ) {
            return;
        }
...
void dGameDisplay_c::setCollect() {
    ...
    for (u32 coinIdx = 0; coinIdx < 3; coinIdx++) {
        if (save->isCollectCoin(w, l, coinIdx) && (dInfo_c::m_startGameInfo.mGameMode == dInfo_c::GAME_MODE_NORMAL)) {
            mpPicturePanes[P_collectOff_00 + 2 * coinIdx]->SetVisible(false);
            mpPicturePanes[P_collection_00 + 2 * coinIdx]->SetVisible(true);
            if (m_3EC[coinIdx] != 2) {
                m_3EC[coinIdx] = 2;
            }
        } else if (dScStage_c::mCollectionCoin[coinIdx] != 4) {
            mpPicturePanes[P_collectOff_00 + 2 * coinIdx]->SetVisible(false);
            mpPicturePanes[P_collection_00 + 2 * coinIdx]->SetVisible(true);
            if (m_3EC[coinIdx] != 2) {
                m_3EC[coinIdx] = 2;
                EffectCollectionCoinGet(coinIdx);
            }
        } else {
            mpPicturePanes[P_collectOff_00 + 2 * coinIdx]->SetVisible(true);
            mpPicturePanes[P_collection_00 + 2 * coinIdx]->SetVisible(false);
            if (m_3EC[coinIdx] != 0) {
                m_3EC[coinIdx] = 0;
            }
        }
