...
void dCc_c::registerCc(dBaseActor_c *actor, sCcDatNewF *collInfo) {
    mpOwner = actor;
    mCcData = *collInfo;
    mFlag = 0;
}

void dCc_c::registerCc(dBaseActor_c *actor, sCcDatNewF *collInfo, u8 nonCollideMask) {
...
bool dCc_c::_hitCheckCircle(dCc_c *c1, dCc_c *c2) {
    ...
    if (distVec.getLength() <= collSizeRadius) {
        ...
        mAng ang = cM::atan2s(distVec.y, EGG::Mathf::abs(distVec.x));
