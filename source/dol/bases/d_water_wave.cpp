...
void dWaterWave_c::wave_tex_set(mVec3_c *pos, float f, int i, u8 a) {
    ...
    mResWave = dResMng_c::m_instance->mRes.getRes(l_bgpoly_arcdt[a], l_bgpoly_bressdt[a]);
    if (m_d308 == 1 || m_d308 == 2) {
        mResDeco = dResMng_c::m_instance->mRes.getRes(l_bgpoly_decarcdt[a], l_bgpoly_decbressdt[a]);
    }
