...
int dCoin_c::execute() {
    ...
    m_shapeAngle.y = m_frame * (0x10000 / COIN_FRAME_COUNT);
    
    if (++m_frame >= COIN_FRAME_COUNT) {
        m_frame = 0;
    }

    m_h_ptn_time--;
    if (m_h_ptn_time == 0) {
        if (++m_h_ptn_no >= ARRAY_SIZE(l_h_block_time)) {
            m_h_ptn_no = 0;
    ...
    m_b_ptn_time--;
    if (m_b_ptn_time == 0) {
        if (++m_b_ptn_no >= ARRAY_SIZE(l_b_block_time)) {
            m_b_ptn_no = 0;
        }
