...
const float dEnemyCarry_c::smc_DIR_MODE0[2] = {2.5f, -2.5f};
const float dEnemyCarry_c::smc_DIR_MODE1[2] = {3.0f, -3.0f};
...
void dEnemyCarry_c::setThrowSpeed(dActor_c *actor) {
    float endSpeed;
    if (mThrowSpeed == 0) {
        endSpeed = smc_DIR_MODE0[mDirection];
    } else {
    ...
    // [Either programmer error or some inline stuff, this never changes the value of endSpeed]
    if (endSpeed < mSpeedMax.x) {
        endSpeed = (endSpeed > mSpeedMax.x) ? mSpeedMax.x : endSpeed;
